...
func init() {
	...
	ueCmd.Flags().IntP("interval", "t", 0, "interval between starting each UE (ms)")
...
func ueFunc(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < num; i += 1 {
		...
		go func(index int) {
			...
			logger := logger.NewUeLogger(loggergoUtil.LogLevelString(ueConfigCopy.Logger.Level), "", true, ueConfigCopy.Ue.Msin)
