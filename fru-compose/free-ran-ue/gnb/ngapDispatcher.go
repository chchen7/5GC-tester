...
func (d *ngapDispatcher) downLinkNASTransportProcessor(g *Gnb, ngapPdu *ngapType.NGAPPDU) {
	...
	for _, ie := range ngapPdu.InitiatingMessage.Value.DownlinkNASTransport.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDNASPDU:
			if ie.Value.NASPDU == nil || len(ie.Value.NASPDU.Value) == 0 {
				g.NgapLog.Errorf("Error downlink NAS transport: NASPDU is empty")
				return
			}
	...
	ranUe := ueValue.(*RanUe)
	if ranUe.GetN1Conn() == nil {
		g.NgapLog.Errorf("Error downlink NAS transport: UE %d N1 connection is nil", ranUeNgapId)
		return
	}
	...
	g.NgapLog.Debugf("Send downlink NAS transport message to UE %s", imsi)
}
